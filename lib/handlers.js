          ...
          if (!err && userData) {
            ...
            _data.delete('users', phone, function (err) {
              if (!err) {
                ...
                // delete each check associated with specified user
                    ...
                    _data.delete('checks', checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, { 'Handlers Error ' : 'Error deleting all checks from user, checks may not have been deleted successfully' });
                        }
                      }
                    });
                  });
  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
        ...
        // verify if given token is valid & belongs to the user creating the check
...
// checks - put
// required data : id, 1of(optional data)
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      _data.read('checks', id, function (err, checkData) {
        if (!err && checkData) {
            ...
            if (tokenIsValid) {
              ...
              _data.update('checks', id, checkData, function (err) {
                if (!err) {
                  ...
                  callback(500, { 'Handlers Error ': 'Could not update the check' });
                }
              });
            } else {
              callback(403);
          ...
          callback(400, { 'Handlers Error ': 'CheckId does not exist' });
        }
      });
    } else {
      callback(400, { 'Handlers Error ': 'Missing field to update' });
    }
  } else {
    callback(400, { 'Handlers Error ': 'Missing required field' });
  }
  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      _data.update('users', checkData.userPhone, userData, function (err) {
                        if (!err) {
                          ...
                          callback(500, { 'Handlers Error ': 'Could not update specified user' });
                        }
                      });
                    } else {
                      callback(500, { 'Handlers Error ': 'Could not find specified check on user object, could not remove check' });
                    }
                  } else {
                    callback(500, { 'Handlers Error ': 'Could not find specified user, could not remove check from list' });
                  }
                });
              } else {
                callback(500, { 'Handlers Error ': 'Could not delete the check data' });
              }
            });
          } else {
            callback(403, { 'Handlers Error ': 'Missing required token in header, or token is invalid' });
        ...
        callback(400, { 'Handlers Error ': ' Specified checkId does not exist' });
      }
    });
  } else {
    callback(400, { 'Handlers Error ': 'Missing required field' });
